...
function TableColumnsTree({ schema, tableName }: { schema: string; tableName: string }) {
  ...
  const showCreateSql = async (item: {
    name: string;
    dataType: string;
    isNullable: boolean;
    defaultValue: string;
  ...
  return (
    <Block flex={false}>
      <ul className={cn('space-y-1', level > 0 && 'ml-8')}>
        {Object.values(columns || ({} as (typeof columns)[string])).map((item) => (
          <li key={item.name}>
              ...
              <ContextMenu>
                ...
                <ContextMenuContent>
                  ...
                  <ContextMenuItem
                    onSelect={async () => {
                      openNewTab({
                        id: `drop/${schema}_${item.name}`,
                        title: `Drop SQL: ${schema}.${item.name}`,
                        type: 'query',
                        queryState: {
                          query: `
                        ...
                        queryComponent: <QueryComponent tabId={`drop/${schema}_${item.name}`} />,
                        resultsComponent: null
                      });
                    }}
                  >
